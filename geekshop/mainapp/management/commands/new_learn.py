...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        action_1__condition = Q(order__update_at__lte=F('order__create_at') + action_1__time_delta)

        action_2__condition = Q(order__update_at__gt=F('order__create_at') + \
                                                  action_1__time_delta) & \
                              Q(order__update_at__lte=F('order__create_at') + \
                                                   action_2__time_delta)

        action_expired__condition = Q(order__update_at__gt=F('order__create_at') + \
                                                        action_2__time_delta)
        ...
        for orderitem in test_orderss:
            t_list.add_row([f'{orderitem.action_order} заказ №{orderitem.pk:}', f'{orderitem.product.name:15}',
                            ...
                            orderitem.order.update_at - orderitem.order.create_at])
            if i == 0:
